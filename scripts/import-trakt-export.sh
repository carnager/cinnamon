...
default_export="${TRAKT_EXPORT:-$HOME/trakt}"
read -r -p "Trakt export directory [$default_export]: " export_dir
export_dir="$(trim "$export_dir")"
...
echo "Importing watched status from Trakt export..."
start_ts="$(date +%s)"
import_body="$(jq -n --arg path "$export_dir" '{path:$path}')"
import_result="$(request POST "$server/api/trakt/import-export" "$import_body" "$token")"
...
printf '%s\n' "$import_response" | jq '{
  ...
  localEpisodesWithIds: .debug.episodeIds,
  exportHistory: .debug.traktSources.exportHistory.items,
  exportMovieRows: .debug.traktSources.exportMovies.items,
  exportEpisodeRows: .debug.traktSources.exportEpisodes.items
}'
echo
